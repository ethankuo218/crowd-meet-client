...
export class DataStore<T> {
  ...
  private networkDelay =
    environment.appShellConfig && environment.appShellConfig.networkDelay
      ? environment.appShellConfig.networkDelay
      : 0;
  ...
  public static AppendShell<T>(
    ...
    return combineLatest([delayObservable, dataObservable]).pipe(
      ...
      startWith({ ...shellModel, isShell: true })
    );
  }

  load(dataSourceObservable: Observable<T>, networkDelay?: number): void {
    const delay =
      typeof networkDelay === 'number' ? networkDelay : this.networkDelay;
    ...
    if (delay === 0) {
      ...
      processedDataSource = DataStore.AppendShell(
    ...
    processedDataSource.subscribe((dataValue: T & ShellModel) => {
